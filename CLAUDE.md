...
The human surface has three levels:

- landing page `/docs/query-library` and provider pages
  `/docs/query-library/<provider>`: React pages generated by the local
  [plugins/query-library-pages](plugins/query-library-pages/index.js) plugin
  from the built index.json (no hand-maintained provider list; provider
  titles/blurbs come from [src/configs/providers.ts](src/configs/providers.ts)
  with a fallback, logos from `static/img/providers/`). These routes use
  `priority: 1` so they beat the docs instance's parent route.
- query pages `/docs/query-library/queries/<id>`: rendered by the third
  content-docs instance (`id: 'query-library'`, autogenerated sidebar grouped
  by provider/service via sidebars-query-library.js). A `DocItem/Content`
  theme wrapper ([src/theme/DocItem/Content](src/theme/DocItem/Content/index.js))
  adds the front-matter-driven metadata panel and related-entries footer on
  query pages only (components in
