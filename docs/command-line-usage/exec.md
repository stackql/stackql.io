...
| Flag | Description |
|--|--|
|<span class="nowrap">`-o`</span><br/><span class="nowrap">`--output`</span>|Output mode, valid values include: `table`(default) `json`, `jsonl` (newline delimited JSON, `ndjson` is accepted as an alias), `csv`, `text` and `otel` (OpenTelemetry log records, see [Output Modes](/docs/getting-started/output-modes#otel-output)), example: `--output jsonl` |
