...
| <span class="nowrap">`-o`, `--output`</span> | string | Output format, must be one of {`json`, `jsonl`, `table`, `csv`, `text`, `otel`} (`ndjson` is accepted as an alias for `jsonl`).  `otel` emits OpenTelemetry log records, see [Output Modes](/docs/getting-started/output-modes#otel-output); available in StackQL releases from `v0.11.660`.  With `OTEL_EXPORTER_OTLP_LOGS_ENDPOINT` (or `OTEL_EXPORTER_OTLP_ENDPOINT`) set, `otel` records are also pushed to that endpoint, see [Pushing to an OTLP endpoint](/docs/getting-started/output-modes#pushing-to-an-otlp-endpoint) | `table` |
